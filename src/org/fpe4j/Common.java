...
class Common {
	 ...
	 */
	static byte[] bitstring(boolean bit, int s) throws IllegalArgumentException {
		// validate s
		if (s < 1)
	 ...
	 *             If X is null.
	 */
	static String byteArrayToHexString(byte[] X) {
		// validate X
		if (X == null)
	 ...
	 */
	static byte[] bytestring(BigInteger x, int s) {
		// validate s
		if (s < 1)
			throw new IllegalArgumentException("s must be a positive integer");
	 ...
	 *             or if x is greater than 256<sup>s</sup>.
	 */
	static byte[] bytestring(int x, int s) {
		// validate s
	 ...
	 *             If X is null.
	 */
	static String intArrayToString(int[] X) {
		// validate X
		if (X == null)
			throw new NullPointerException("X must not be null");
	 ...
	 */
	static BigInteger num(int[] X, int radix) {
		// validate X
		if (X == null)
			throw new NullPointerException("X must not be null.");
		if (X.length < 1 || X.length > Constants.MAXLEN)
	 ...
	 */
	static int[] str(BigInteger x, int radix, int m) {
		// validate m
	 ...
	 *             If X is null.
	 */
	static String unsignedByteArrayToString(byte[] X) {
		// validate X
		if (X == null)
			throw new NullPointerException("X must not be null");
